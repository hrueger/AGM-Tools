...
export class FilesComponent implements OnInit {
    ...
    public pid: number;
    public settings = {
        chunkSize: 1024 * 1024, saveUrl: `${this.environmentService.environment.apiUrl}files/upload`,
    };
    ...
    public share(item, shareModal, shareDropFolder = false) {
        ...
        this.modalService
            .open(shareModal);
        if (shareDropFolder) {
            this.shareLink = `${this.environmentService.environment.appUrl}upload/${item.id}/${item.dropFolder.title.replace(/ /g, "-")}`;
        } else {
            this.remoteService
                .getNoCache("post", `files/${item.id}/share${shareDropFolder ? "DropFolder" : ""}`)
                .subscribe((data) => {
                    if (data.status == true) {
                        this.shareLink = `${this.environmentService.environment.appUrl}share/${data.link}`;
                    }
                });
        }
    }
        ...
        this.documentEditorConfig = {
            editorConfig: {
                ...
                editorConfig: {
                    ...
                    callbackUrl: `${this.environmentService.environment.apiUrl}files/track`,
                    /* embedded: {
            ...
            script: `${this.environmentService.environment.documentServerUrl}/web-apps/apps/api/documents/api.js`,
        };
    }
